...
## Signed entries (authenticity layer)
Each node generates an Ed25519 keypair on first run (private key in `.node_key`,
git-ignored, never synced). `node.register_key()` publishes the PUBLIC key to
central (trust-on-first-use pinning). Every appended entry is signed; central
verifies signatures on push and rejects forgeries. `verify_chain(entries, public_key=...)`
checks authenticity in addition to integrity and lineage.

## Dashboard
The central server serves an operator console at `/`:
- live node list with signed/unsigned lock indicator
- click a node to inspect its full chain (seq, id↔parent links, payload, state digest, signature status)
- one-click server-side chain verification with pass/fail badge
- live stats (entries, nodes, keys pinned), auto-refreshing
Open http://localhost:8000/ after starting uvicorn.
